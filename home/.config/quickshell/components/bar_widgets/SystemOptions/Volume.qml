...
Rectangle {
	...
	readonly property var audioSinks: {
		...
		for (var i = 0; i < nodes.length; i++) {
			var node = nodes[i]
			if (node.isSink && !node.isStream) result.push(node)
		}
		return result
	}
	...
	onPipewireVolumeChanged: {
		if (!sliderMouse.pressed) currentVolume = pipewireVolume
	}
	...
	function setVolumeFromTrack(mouseX: real, trackWidth: real): void {
		...
		if (sink && sink.audio) sink.audio.volume = root.currentVolume / 100
	}
				...
				RowLayout {
					...
					Item {
						...
						Text {
							...
							text: !sink || !sink.audio ? "󰝟"
								: (sink.audio.muted ? "󰝟" : (root.currentVolume > 50 ? "󰕾" : "󰖀"))
					...
					Item {
						...
						Rectangle {
							...
							x: Math.max(0, Math.min(sliderTrack.width - width,
								Math.round(sliderTrack.width * root.currentVolume / 100) - width / 2))
					...
					Rectangle {
						...
						implicitHeight: Theme.bar_widget_height
						radius: Theme.radius_normal
						color: root.expanded || deviceToggle.pressed
							? Theme.color_surface_pressed
							: (deviceToggle.hovered ? Theme.color_surface_hover : "transparent")
				...
				ColumnLayout {
					...
					Repeater {
						...
						Rectangle {
							...
							radius: Theme.radius_normal
							color: deviceItem.pressed
								? Theme.color_surface_pressed
								: (deviceItem.hovered || deviceItem.isDefault ? Theme.color_surface_hover : "transparent")
