...
usage() {
	...
	printf 'Sends a notification with both image and text and verifies the notification region changes.\n'
}
...
gdbus call --session \
	...
	"" \
	"Silly little test" \
	"Image and text should both render on ${NOTIFICATION_TEST_MONITOR_NAME}" \
	"[]" \
	"{'image-path': <'$image_path'>}" \
	2500 >/dev/null

notifications_assert_changed "PASS: image-and-text notification rendered"
