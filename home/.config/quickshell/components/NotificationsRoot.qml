...
Scope {
	...
	Variants {
		...
		PanelWindow {
			...
			readonly property int topOffset: root.margin
				+ Theme.bar_widget_height
				+ (Theme.bar_padding * 2)
				+ root.stack_gap_below_bar
				...
				Column {
					...
					Repeater {
						...
						delegate: Item {
							...
							property bool shouldAutoExpire: (!notification.resident || root.expire_resident)
								&& (notification.urgency !== NotificationUrgency.Critical || root.expire_critical)
							property int resolvedTimeout: {
								if (!root.use_notification_timeout) return root.default_timeout_ms;
								if (notification.expireTimeout <= 0) return root.default_timeout_ms;
								return notification.expireTimeout;
							}
							implicitWidth: root.width
							implicitHeight: Math.max(
								root.min_height,
								content.implicitHeight + (root.padding * 2) + root.top_accent_height
							)
							width: implicitWidth
									...
									Rectangle {
										...
										implicitHeight: notificationImage.status === Image.Ready
											? Math.min(
												...
												(notificationImage.implicitHeight > 0 ? notificationImage.implicitHeight : root.image_max_height)
											)
											: root.image_max_height
